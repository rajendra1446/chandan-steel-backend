...
export const queryAiMetallurgical = async (queryText = "", explicitParams = {}) => {
    ...
    // 1. Dynamic Billet lookup (via explicitParam, database search, or regex)
    ...
    // 2. Dynamic Heat lookup (via explicitParam, database search, or regex)
    ...
    // 3. Dynamic Grade lookup directly from DB
    ...
    // 4. Fallback to full plant overview
    const overview = await getPlantAiOverview();

    // Check specific topic intent
    if (q.includes("remain") || q.includes("consume") || q.includes("balance") || q.includes("how many billet")) {
        return {
            type: "billet_balance",
            answer: "Live Billet Balance: Consumed vs Remaining Yard Stock",
    ...
    if (q.includes("product") || q.includes("built") || q.includes("output")) {
        return {
            type: "product_output",
    ...
    return {
        type: "plant_overview",
        answer: "Here is the dynamic plant-wide metallurgical overview:",
